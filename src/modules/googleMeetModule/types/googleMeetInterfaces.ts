...
export interface GoogleMeetUtilsInterface extends GeneralServiceInterface {
   ...
   leadsNoLeads(list: exitConfigListType): {
      leads: string[];
      noLeads: string[];
   };
   myRole(list: exitConfigListType);
   disableParticipants(
      participantsBtn: Element,
      disabledParticipants: boolean,
   ): void;
...
export interface GoogleMeetConditionInterface extends GeneralServiceInterface {
   ...
   leaveWithMyFriendsHandle(
      meetingsData: MeetingsData | boolean,
      rules: MeetTopic['rules'],
      party: string[],
   ): void | boolean;
}
...
export interface GoogleMeetChromeStorageInterface
   extends GeneralServiceInterface {
   getMeetingsData(
      title: GoogleMeetTopics[number] | 'Topic is absent',
      meetingsData: MeetingsData | boolean,
      exitConfigList: Omit<Participant, 'assignedTo'>[],
   ): void;
}
...
export interface GoogleMeetDOMManipulationsInterface {
   disableCamera(belowSectionWithBtnExtMeet: Element): Promise<boolean>;
